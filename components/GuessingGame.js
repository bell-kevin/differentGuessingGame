...
const GuessingGame = () => {
    ...
    const handleGuessSubmission = () => {
        ...
        if (guessedNumber === targetNumber) {
            ...
            setGuessesFeedback([...guessesFeedback, guessedNumber > targetNumber ? 'Too high' : 'Too low']);
        }
            ...
            {guesses.length > 0 && (
                <View>
                    ...
                    {guesses.map((guess, index) => (
                        <View key={index}>
                            <Text style={styles.textAlignCenter}>{`${index + 1}. ${guess}`}</Text>
                            {guesses[index] !== targetNumber && (
